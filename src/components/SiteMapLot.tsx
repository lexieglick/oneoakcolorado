...
import sitemapImage from "@/assets/sitemap-bw.png";
      ...
      <main className="pt-24">
        ...
        <section className="px-6 pb-24">
          <div className="mx-auto max-w-4xl">
            ...
            <div className="flex items-center justify-center gap-8 mb-10">
              {Object.entries(statusMeta).map(([status, { label, swatch }]) => (
                <div key={status} className="flex items-center gap-2.5">
                  <div className="w-3 h-3 rounded-sm" style={{ backgroundColor: swatch }} />
            ...
            >
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      border: isHovered ? "1px solid rgba(0,0,0,0.2)" : "1px solid transparent",
                    }}
              ...
              {hoveredLot && (() => {
                ...
                return (
                  ...
                  >
                    <p className="text-xs font-display whitespace-nowrap" style={{ color: "#1C1A18" }}>
                      {lot.name}
                    </p>
            ...
            {/* Lot directory */}
            <div
              className="mt-16 grid grid-cols-1 md:grid-cols-2 gap-px rounded-sm overflow-hidden"
              style={{ background: "#D6C9B4", border: "1px solid #D6C9B4" }}
            >
              {["B1", "B2"].map((block) => (
                <div key={block} className="p-8" style={{ backgroundColor: "#F7F3ED" }}>
                  <h3 className="font-display text-lg mb-4" style={{ color: "#4A3F33" }}>
                    Block {block}
                  </h3>
                    ...
                    {lots
                      ...
                      .map((lot) => (
                        <button
                          key={lot.id}
                          onClick={() => setSelectedLot(lot)}
                          className="flex items-center justify-between w-full py-2 px-3 rounded-sm text-left transition-colors"
                          style={{ backgroundColor: "transparent" }}
                          onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = "#EDE8DF")}
                          onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = "transparent")}
                        >
                          <div className="flex items-center gap-3">
                            <span className="text-xs w-5" style={{ color: "#BCA98C" }}>
                              {lot.number}
                            </span>
                            <span className="text-sm" style={{ color: "#4A3F33" }}>
                              {lot.name}
                            </span>
                          </div>
                          <span
